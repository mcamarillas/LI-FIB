...
writeClauses:- 
    teamExactlyKMembers,
    eachStudentConsidersFriends,
    exactlyOneExpert,
    true, !.
writeClauses:- told, nl, write('writeClauses failed!'), nl,nl, halt.

teamExactlyKMembers:- numMembersTeam(N), findall(chosen(S), student(S), Lits), exactly(N, Lits),fail.
teamExactlyKMembers.

eachConsidersFirends:- info(S,_,_).
eachConsidersFirends.

exactlyOneExpert:- info(_,_,E), findall(chosen(S), expertise(S,E), Lits), exactly(1,Lits).
atMostOneExpert.
