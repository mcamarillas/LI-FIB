...
oneCityPerPosition:- position(P), findall(visited(C,P), city(C), Lits), exactly(1,Lits), fail.
oneCityPerPosition.

onePositionPerCity:- city(C), C \= 1, findall(visited(C,P), position(P), Lits), exactly(1,Lits), fail.
onePositionPerCity.

city1FirstVisited:- writeClause([visited(1,0)]).
city1FirstVisited.

city1LastVisited:- writeClause([visited(1,20)]).
city1LastVisited.

cityAdjacency:- city(C2), adjacency(C1,L), position(P2), position(P1), P1 is P2 - 1, not(member(C2,L)),
                writeClause([ -visited(C1,P1), -visited(C2,P2) ]), fail.
cityAdjacency.

costRoad:- position(P1), position(P2), P1 is P2 - 1, city(C1), city(C2), 1 is (C1 + C2) mod 2,
           writeClause([ -visited(C1,P1), -visited(C2,P2), cost(P1)]), fail.
costRoad.

costAtMostMaxCost:- maxCost(M), findall(cost(P), position(P), Lits), atMost(M,Lits).
costAtMostMaxCost.
