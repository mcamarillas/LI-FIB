...
atLeastOneGasAtRoad:- road(C1, C2), writeClause([install(C1), install(C2)]), fail.
atLeastOneGasAtRoad.

asMuchMaxStations:- findall(install(C), city(C), Lits), atMost(14, Lits).
asMuchMaxStations.
...
displaySol(M):- nl, city(C), member(install(C), M), write(C), nl, fail.
displaySol(_):- nl, nl.
