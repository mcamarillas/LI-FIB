...
satVariable( taskTime(T,H) ):- task(T), integer(H).  %"task T is being executed at hour H"
...
writeClauses(MaxHours):-
    ...
    noSolapationMaterials(MaxHours),
    true,!.
writeClauses(_):- told, nl, write('writeClauses failed!'), nl,nl, halt.

eachTaskStartsOnce(MaxHours):- 
	duration(T,D), MaxStart is MaxHours - D + 1, 
	findall(start(T,H1), between(1,MaxStart,H1), Lits), exactly(1,Lits), fail.
eachTaskStartsOnce(_).

defineTaskTime(MaxHours):- duration(T,D), 
	MaxStart is MaxHours - D + 1, between(1,MaxStart,Start), D1 is Start + D -1, between(Start,D1,D2), writeClause([-start(T,Start), taskTime(T,D2)]), fail.
defineTaskTime(_).

noSolapationMaterials(MaxHours):-
	resourceUnits(R,U), between(1,MaxHours,H), findall(taskTime(T,H), usesResource(T,R), Lits) , atMost(U,Lits), fail.
noSolapationMaterials(_).
