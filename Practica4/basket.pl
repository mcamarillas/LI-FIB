...
eachTeamAgainstExactlyOne:- difTeams(S,T),
    findall(match(S,T,R), round(R), LitsH),
    findall(match(T,S,R), round(R), LitsA), append(LitsH,LitsA,Lits), exactly(1,Lits), fail.
eachTeamAgainstExactlyOne.

atLeastOneTvTeam:- round(R),          
    findall( match(S,T,R), tvMatch(S,T), LitsH ),
    findall( match(T,S,R), tvMatch(S,T), LitsA ), append(LitsH,LitsA,Lits), atLeast(1,Lits), fail.
atLeastOneTvTeam.

certainRoundNotHome:- team(T), round(R),
	away(T,R), writeClause([-home(T,R)]), fail.
certainRoundNotHome.

defineHome:- difTeams(S,T), round(R), 
	writeClause([-match(S,T,R), home(S,R)]), writeClause([-match(T,S,R), home(T,R)]), fail.
defineHome.

defineDoubles:- team(T), round(R), R1 is R + 1, round(R1), 
	writeClause([-home(T,R), -home(T,R1), double(T,R)]),
	writeClause([home(T,R), home(T,R1), double(T,R)]), fail.
defineDoubles.
