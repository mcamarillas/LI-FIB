...
incompatible(S1,S2):- %% returns a pair of students that coincided last year
    student(S1), student(S2), S1 < S2, lastYearTeam(L), member(S1,L), member(S2,L).

%%%%%%  SAT Variables:
satVariable( st(S,T) ):- student(S), team(T). % student S goes to team T

writeClauses:- 
	atMostOneIncompatible,
	teamSizeIsN,
	studentNotRepited,
    true,!.                    % this way you can comment out ANY previous line of writeClauses
writeClauses:- told, nl, write('writeClauses failed!'), nl,nl, halt.

studentNotRepited:- student(S1), team(T), team(T1), T1 \= T, writeClause( [-st(S1,T),-st(S1,T1)] ), fail.
studentNotRepited.

teamSizeIsN:- teamSize(N), team(T), findall(st(S,T), student(S), Lits), exactly(N,Lits), fail.
teamSizeIsN.

atMostOneIncompatible:- student(S1), team(T), findall(st(S2,T), incompatible(S1,S2), Lits), atMost(1,Lits), fail.
atMostOneIncompatible.
