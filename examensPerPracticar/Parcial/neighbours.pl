...
writeClauses:- 
	exactlyKRepresentatives,
	atMostOneRepresentativePerNeihborhood,
	allNeighborsRepresented,
    true,!.                    % this way you can comment out ANY previous line of writeClauses
writeClauses:- told, nl, write('writeClauses failed!'), nl,nl, halt.

exactlyKRepresentatives:- numRepresentatives(K), 
						  findall( chosen(N), neighbor(N), Lits ), 
						  exactly(K,Lits), fail.
exactlyKRepresentatives.

atMostOneRepresentativePerNeihborhood:- info(_,_,NH), 
										findall( chosen(N), neighborNeighborhood(N,NH), Lits ), 
										atMost(1,Lits), fail. 
atMostOneRepresentativePerNeihborhood.

allNeighborsRepresented:- neighborRepresentedBy(_,L),
						  findall( chosen(N1), member(N1, L), Lits), atLeast(1,Lits), fail.
allNeighborsRepresented.
