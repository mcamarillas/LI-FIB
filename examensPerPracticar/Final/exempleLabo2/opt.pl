...
maxTwoDoublesPerTeam:- 
	team(T), findall(double(T,R), round(R), Lits), atMost(2,Lits), fail.
maxTwoDoublesPerTeam.

maxOneTvMatchFirstTenRounds:-
	round(R), R < 11, findall( match(S,T,R), tvMatch(S,T), Lits ), atMost(1,Lits), fail.
maxOneTvMatchFirstTenRounds.
...
twoDoublesImpliesHasTwoDoubles:- 
	team(T), round(R), round(R1), R \= R1,  writeClause([ -double(T,R), -double(T,R1), hasTwoDoubles(T) ]), fail.
twoDoublesImpliesHasTwoDoubles.
...
% solution of this exam:
costOfThisSolution(M,Cost):- between(0,20,I), Cost is 20-I, findall(S ,member(hasTwoDoubles(S),M),L), length(L,Cost), !.
